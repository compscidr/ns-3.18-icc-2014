 ...
 * This experiment is the same as exp1 but now uses two APs so that
 * the clients have the choice of AP. (but still on same channel)
 *
...
int main(int argc, char * argv[])
{
	...
	for(run = 0; run < runs; run++)
	{
		...
		string phyModeFast ("DsssRate11Mbps");
		...
		//NetDeviceContainer apWifiDevs = wifiAP.Install (wifiPhy, wifiMac, aps); //only use this when we want all the APs the same phyMode
		
		//Add each phymode one at a time for multiple APs
		NetDeviceContainer apWifiDevs;
		apWifiDevs.Add(wifiAP.Install(wifiPhy, wifiMac, aps.Get(0)));
		...
		apWifiDevs.Add(wifiAP.Install(wifiPhy, wifiMac, aps.Get(1)));
		...
		for(unsigned int i = 0; i < clientInterfaces.GetN(); i++)
		{
			...
			videoSource.SetConstantRate (DataRate ("0.5Mbps"));
